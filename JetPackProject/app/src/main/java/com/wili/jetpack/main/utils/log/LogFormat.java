...
public class LogFormat {
    ...
    public static String formatBorder(String[] segments) {
        ...
        msgBuilder.append(TOP_HORIZONTAL_BORDER).append(XPrinter.getLineSeparator());
        for (int i = 0; i < nonNullCount; i++) {
            msgBuilder.append(appendVerticalBorder(nonNullSegments[i]));
            if (i != nonNullCount - 1) {
                msgBuilder.append(XPrinter.getLineSeparator()).append(DIVIDER_HORIZONTAL_BORDER)
                        .append(XPrinter.getLineSeparator());
            } else {
                msgBuilder.append(XPrinter.getLineSeparator()).append(BOTTOM_HORIZONTAL_BORDER);
            }
        }
    ...
    private static String appendVerticalBorder(String msg) {
        ...
        String[] lines = msg.split(XPrinter.getLineSeparator());
        for (int i = 0, N = lines.length; i < N; i++) {
            if (i != 0) {
                borderedMsgBuilder.append(XPrinter.getLineSeparator());
            }
            String line = lines[i];
